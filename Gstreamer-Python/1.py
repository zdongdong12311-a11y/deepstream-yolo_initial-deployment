        ...
        if result.returncode == 0:
            ...
            for line in lines:
                if 'Size:' in line:
                    # 解析分辨率
                    import re
                    match = re.search(r'Size:\s*(\d+)x(\d+)', line)
                    if match:
                        width, height = int(match.group(1)), int(match.group(2))
                        if (width, height) not in supported_resolutions:
                            supported_resolutions.append((width, height))
            
            print(f"✅ 检测到 {len(supported_resolutions)} 种支持的分辨率")
        else:
            print("⚠️  无法自动检测摄像头能力，使用默认分辨率")
            
    except Exception as e:
        print(f"⚠️  检测摄像头能力时出错: {e}")
    
    return supported_resolutions

def select_resolution(device):
    """让用户选择分辨率"""
    # 首先检测摄像头支持的分辨率
    supported_resolutions = detect_camera_capabilities(device)
    
    # 定义可用的分辨率选项
    RESOLUTION_OPTIONS = {
        "1": {"width": 320, "height": 240, "name": "320x240 (低资源)"},
        "2": {"width": 640, "height": 480, "name": "640x480 (推荐)"},
        "3": {"width": 1280, "height": 720, "name": "1280x720 (高清)"},
        "4": {"width": 1920, "height": 1080, "name": "1920x1080 (全高清)"}
    }
    ...
    for key, value in RESOLUTION_OPTIONS.items():
        ...
        if not supported_resolutions or resolution in supported_resolutions:
            available_options[key] = value
        else:
            print(f"⚠️  分辨率 {value['name']} 可能不被摄像头支持")
            available_options[key] = value  # 仍然提供选项，但给出警告
    
    print("\n📺 请选择分辨率:")
    print("-" * 50)
    for key, value in available_options.items():
        support_status = "✅" if not supported_resolutions or (value["width"], value["height"]) in supported_resolutions else "⚠️ "
        print(f"  {key}. {value['name']} {support_status}")
    
    while True:
        choice = input("\n请输入选择 (1-4, 默认为2): ").strip()
        if choice == "":
            choice = "2"
        if choice in available_options:
            selected = available_options[choice]
            print(f"✅ 选择分辨率: {selected['name']}")
            
            # 根据分辨率调整帧率
            if selected["width"] >= 1920:
                print("💡 高分辨率模式，建议使用较低帧率以保证稳定性")
            elif selected["width"] <= 320:
                print("💡 低分辨率模式，可以尝试更高帧率")
                
            return selected['width'], selected['height']
        else:
            print("❌ 无效选择，请重新输入")

def select_fps(width, height):
    """根据分辨率选择帧率"""
    if width >= 1920:
        # 高分辨率使用较低帧率
        FPS_OPTIONS = {
            "1": {"fps": 15, "name": "15fps (稳定)"},
            "2": {"fps": 20, "name": "20fps (平衡)"},
            "3": {"fps": 30, "name": "30fps (高性能)"}
        }
        default_choice = "1"
    elif width >= 1280:
        FPS_OPTIONS = {
            "1": {"fps": 20, "name": "20fps (稳定)"},
            "2": {"fps": 25, "name": "25fps (平衡)"},
            "3": {"fps": 30, "name": "30fps (推荐)"}
        }
        default_choice = "3"
    else:
        # 低分辨率可以使用更高帧率
        FPS_OPTIONS = {
            "1": {"fps": 30, "name": "30fps (稳定)"},
            "2": {"fps": 45, "name": "45fps (平衡)"},
            "3": {"fps": 60, "name": "60fps (高性能)"}
        }
        default_choice = "2"
    
    print(f"\n🎯 请选择帧率 ({width}x{height}):")
    print("-" * 40)
    for key, value in FPS_OPTIONS.items():
        print(f"  {key}. {value['name']}")
    
    while True:
        choice = input(f"\n请输入选择 (1-3, 默认为{default_choice}): ").strip()
        if choice == "":
            choice = default_choice
        if choice in FPS_OPTIONS:
            selected = FPS_OPTIONS[choice]
            print(f"✅ 选择帧率: {selected['name']}")
            return selected['fps']
        else:
            print("❌ 无效选择，请重新输入")
...
def main(args):
    ...
    print("🚀 启动 DeepStream 6.0 检测器 - NX 优化版")
    print("=" * 60)

    # 让用户选择分辨率和帧率
    width, height = select_resolution(device)
    fps = select_fps(width, height)
